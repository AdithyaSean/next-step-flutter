...
class Students extends Table {
  ...
  TextColumn get predictions => text().map(const ListConverter<Map<String, dynamic>>())();
...
class StudyStreams extends Table {
  ...
  TextColumn get possibleCourses => text().map(const ListConverter<Map<String, dynamic>>())();
...
class AppDatabase extends _$AppDatabase {
  ...
  // Student operations
  Future<Student?> getStudent(String id) => (select(students)..where((t) => t.id.equals(id))).getSingleOrNull();
  Future<void> insertStudent(Student student) => into(students).insert(student);
  Future<void> updateStudent(Student student) => (update(students)..where((t) => t.id.equals(student.id))).write(student);
  Future<void> deleteStudent(String id) => (delete(students)..where((t) => t.id.equals(id))).go();

  // Stream operations
  Future<StudyStream?> getStream(String id) => (select(studyStreams)..where((t) => t.id.equals(id))).getSingleOrNull();
  Future<void> insertStream(StudyStream stream) => into(studyStreams).insert(stream);
  Future<void> updateStream(StudyStream stream) => (update(studyStreams)..where((t) => t.id.equals(stream.id))).write(stream);
  Future<void> deleteStream(String id) => (delete(studyStreams)..where((t) => t.id.equals(id))).go();

  // Course operations
  Future<Course?> getCourse(String id) => (select(courses)..where((t) => t.id.equals(id))).getSingleOrNull();
  Future<void> insertCourse(Course course) => into(courses).insert(course);
  Future<void> updateCourse(Course course) => (update(courses)..where((t) => t.id.equals(course.id))).write(course);
  Future<void> deleteCourse(String id) => (delete(courses)..where((t) => t.id.equals(id))).go();
}
